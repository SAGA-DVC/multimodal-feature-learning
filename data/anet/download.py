'''
Taken from https://github.com/skasai5296/actnetchallenge/blob/master/dataset/preparedataset.py
'''
...
def urlgenerator(obj):
    for key, value in obj.items():
            yield key, f'https://www.youtube.com/watch?v={key[2:]}', value
...
def downloader(download_path, urlgen):
    ...
    failed_ids = []
    before = time.time()
    for id, url, annotation in urlgen:
        try:
            yt = YouTube(url)
            st = yt.streams.filter(file_extension='mp4').first()
            st.download(output_path=download_path, filename=f"{id}.mp4")
            succeedcnt += 1
            succeeded[id] = annotation
