...
# Source: https://github.com/HumamAlwassel/TSP/blob/master/data/standardize_videos_to_constant_30fps_mp4.sh
...
do
	...
	echo "ffmpeg -y -i $input_video_path -filter:v fps=fps=30 -c:a copy $output_video_path"
	ffmpeg -y -i $input_video_path -filter:v fps=fps=30 -c:a copy $output_video_path
done
