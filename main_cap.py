...
from engine_cap import train_one_epoch, evaluate
...
def train_cap(args):
    ...
    model, criterion = build_model_and_criterion(args.procedure, args.dvc, dataset_train, args.use_differentiable_mask)
    model.to(device)
    criterion.to(device)
    ...
    if not args.only_eval:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if args.output_dir:
                checkpoint_paths = [output_dir / 'checkpoint_cap.pth']
                # extra checkpoint before LR drop and every 100 epochs
                if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % args.checkpoint_rate == 0:
                    checkpoint_paths.append(output_dir / f'checkpoint_cap{epoch:04}.pth')
            ...
            if args.output_dir and is_main_process():
                with (output_dir / "train_log_cap.txt").open("a") as f:
                    f.write(json.dumps(train_log_stats) + "\n")
                
                with (output_dir / "val_log_cap.txt").open("a") as f:
                    f.write(json.dumps(val_log_stats) + "\n")

                if args.wandb.on:
                    wandb.save(os.path.join(output_dir, "train_log_cap.txt"))
                    wandb.save(os.path.join(output_dir, "val_log_cap.txt"))
