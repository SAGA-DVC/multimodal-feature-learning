...
class TSPModel(nn.Module):
    ...
    If `num_tsp_heads` is `None` or 0, then combined features from backbone(s) are returned after a Linear layer, irrespective of `return_features` parameter of the forward method
    ...
    def __init__(self, backbones, input_modalities, d_feat, d_tsp_feat,  num_tsp_classes, num_tsp_heads=1, concat_gvf=False, combiner=None):
        '''
        Args:
            backbone (List[torch.nn.Module]): One or more backbone architectures
            d_feat (int): The dimension for input to TSP layers (output dimension of combiner function)
            d_tsp_feat (int): The dimension for TSP features
        ...
        # TODO This will be an untrained layer for pre-TSP feature extraction
        # Make it optional?
        self.fc = nn.Linear(self.d_feat, self.d_tsp_feat)
    ...
    def forward(self, x, gvf=None, return_features=False):
        ...
        tsp_features = self.fc(features)
        if self.num_heads is None:
            return tsp_features

        if self.num_heads == 1:
            logits = [self.action_fc(tsp_features)]
            return (logits, tsp_features) if return_features else logits

        elif self.num_heads == 2:
            logits = [self.action_fc(tsp_features)]
            if self.concat_gvf:
                assert gvf is not None, "Forward pass expects a global video feature input but got None"
                logits.append(self.region_fc(torch.cat([tsp_features, gvf], dim=-1)))
            else:
                logits.append(self.region_fc(tsp_features))
