...
from ..modules.misc_modules import decide_two_stage, predict_event_num
...
# TODO - aux loss multiple embedding heads add
class UnimodalDeformableDVC(nn.Module):
    ...
    def forward(self, obj, is_training=True, faster_eval=False):
        ...
        for lvl in range(num_pred):
            ...
            output_segment = self.segment_embedding[lvl](query_features[lvl]).sigmoid()
            ...
            outputs_segments.append(output_segment)

        outputs_class = torch.stack(outputs_classes)    # (1, batch_size, num_queries, num_classes + 1) OR (depth, batch_size, num_queries, num_classes + 1)
        outputs_count = torch.stack(outputs_counts)    # (1, batch_size, max_eseq_length + 1) OR (depth, batch_size, max_eseq_length + 1)
        outputs_segment = torch.stack(outputs_segments)     # (1, batch_size, num_queries, 2) OR (depth, batch_size, num_queries, 2)

        out = {'pred_logits': outputs_class[-1], 'pred_segments': outputs_segment[-1], 'pred_count': outputs_count[-1]}

        # indices
        # scores, labels = outputs_class[-1].max(-1)
        # bool_indices = scores > self.threshold    # (batch_size, num_queries)

        # for i, bool_index in enumerate(bool_indices):
        #     if bool_index.sum() == 0:    # all False
        #         _, topk_labels = scores[i].topk(k=3, dim=-1)
        #         bool_indices[i, topk_labels] = True

        # indices = self.convert_bool_to_int_indices(bool_indices)    # list of len=batch_size [(pred_num_segments)] OR [(3)]
        ...
        for lvl in range(num_pred):
            ...
            denormalized_segments = denormalize_segments(out_aux['pred_segments'][idx], video_durations, idx[0])
            ...
            if self.use_differentiable_mask:
                ...
                query_features_selected_segments = query_features[lvl][idx]  # (nb_target_segments, d_model)
        ...
        # Inference
            ...
            for word_index in range(1, self.seq_len - 1):
                ...
                for lvl in range(num_pred):
                    # (1, total_caption_num, max_caption_length - 1, vocab_size) OR (depth, total_caption_num, max_caption_length - 1, vocab_size)
                    if self.use_differentiable_mask:
                        output_caption_val = self.unimodal_caption_decoder(captions, memory_list[lvl], tgt_mask, captions_padding_mask, pred_memory_mask_list[lvl])
                    else:
                        output_caption_val = self.unimodal_caption_decoder(captions, memory_list[lvl], tgt_mask, captions_padding_mask, memory_mask_list[lvl])
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_segment, outputs_count, outputs_caption):
    ...
    def convert_bool_to_int_indices(self, bool_indices):
        """
        Parameters:
            `bool_indices`: Tensor of dimension (batch_size, num_queries)
        
        Returns:
            `indices`: list (len=batch_size) of tuple of tensors (tuple dimensions=(2, gt_target_segments))
        """
        indices = []
        for bool_index in bool_indices:
            index = bool_index.nonzero().flatten()    # (predicted_segments)
