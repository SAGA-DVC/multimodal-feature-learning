...
class UnimodalSparseDVC(nn.Module):
    ...
    def forward(self, obj, is_training=True, faster_eval=False):
        ...
        if is_training:
            ...
            if self.use_differentiable_mask:
                return out, outputs_caption_last_layer, indices, indices_aux, torch.squeeze(memory_mask_list[-1]).float()
            else:
        ...
        # Inference
            ...
            if self.use_differentiable_mask:
                return out, captions_with_eos, indices, indices_aux, torch.squeeze(memory_mask_list[-1]).float()
            else:
