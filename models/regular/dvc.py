...
from ..modules.embedding_layers import PositionalEmbedding, PositionEmbeddingVideoSine
...
class DVC(nn.Module):
    def __init__(self, input_modalities, num_queries, d_model, num_classes, aux_loss, matcher, threshold, max_eseq_length,
                ...
                decoder_args, caption_args, use_differentiable_mask=False):
        ...
        # Context Module
        self.video_rescale_len = decoder_args.video_rescale_len
        
        self.use_differentiable_mask = use_differentiable_mask
        if use_differentiable_mask:
            self.context_mask_model = ContextMaskModel(in_dim=(2 + d_model), out_dim=(self.video_rescale_len))
    ...
    def forward(self, obj, is_training=True, faster_eval=False):
        ...
        # Decoder
        # query_embedding_weight = self.query_embedding.weight.unsqueeze(0).repeat(batch_size, 1, 1)    # (batch_size, num_queries, d_model)
        # target = torch.zeros_like(query_embedding_weight)

        query_embedding_weight = self.query_embedding.weight
        query_embed, target = torch.chunk(query_embedding_weight, 2, dim=1)    # target->(num_queries, d_model)  query_embed->(num_queries, d_model)
        query_embed = query_embed.unsqueeze(0).expand(batch_size, -1, -1)    # (batch_size, num_queries, d_model)  
        target = target.unsqueeze(0).expand(batch_size, -1, -1)     # (batch_size, num_queries, d_model)

        # (1, batch_size, num_queries, d_model) OR # (depth, batch_size, num_queries, d_model)
        query_features = self.decoder(target=target, memory=video, positional_embedding=positional_embedding, query_embedding=query_embed, target_mask=None, memory_mask=video_mask.unsqueeze(1).unsqueeze(1))
        ...
        memory, memory_mask = self.get_segment_features(video, denormalized_segments, idx, video_durations)
    ...
    def crop_segments(self, features, denormalized_segments, segment_batch_id, video_durations):
        ...
        lower_limit = 0
        upper_limit = self.video_rescale_len
