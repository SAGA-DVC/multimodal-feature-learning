...
class VideoVisionTransformer(nn.Module):
    def __init__(self, model_name, num_frames, num_patches, img_size=224, spatial_patch_size=16, temporal_patch_size=1,
        ...
        if model_name not in ['factorised encoder', 'factorised self attention', 'factorised dot product attention'] :
        ...
        if weight_init and model_official is not None:
            self.load_weights(self, model_official, model_name)

    @staticmethod
    def load_weights(model_custom, model_official, model_name):
        ...
        These weights include token embeddings, positional embeddings, [class] tokens (if any), encoder blocks and the classifier layer.
        ...
        VideoVisionTransformer.load_token_embeddings(model_custom, model_official)
        VideoVisionTransformer.load_positional_embeddings(model_custom, model_official, model_name)
        if model_name == 'factorised encoder' :
            VideoVisionTransformer.load_cls_tokens(model_custom, model_official)
    ...
    def load_encoder_weights(model_custom, model_official, model_name):
        ...
        if model_name == 'factorised encoder':
            ...
            for (name_custom, parameter_custom), (name_official, parameter_official) in zip(
                list(model_custom.named_parameters())[6:144+4], list(model_official.named_parameters())[4:144+4]):
            ...
            #temporal
            for (name_custom, parameter_custom), (name_official, parameter_official) in zip(
                list(model_custom.named_parameters())[144+6:144+144+6], list(model_official.named_parameters())[4:144+4]):
