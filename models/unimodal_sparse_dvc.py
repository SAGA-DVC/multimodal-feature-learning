...
from .modules.misc_modules import decide_two_stage, inverse_sigmoid
...
class UnimodalSparseDVC(nn.Module):
    ...
    def forward(self, obj, is_training=True, faster_eval=False):
        ...
        outputs_class = []
        outputs_segment = []
        for lvl in range(query_features.shape[0]):
        ...
        outputs_segment = torch.stack(outputs_segment)
        ...
        if self.use_enc_aux_loss:
            out['aux_outputs_enc'] = self._set_aux_loss(enc_inter_outputs_class, enc_inter_outputs_segments)
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_segment, outputs_captions=None):
        if outputs_captions is None:
            return [{'pred_logits': a, 'pred_segments': b,}
                for a, b in zip(outputs_class, outputs_segment)]

        else:
            return [{'pred_logits': a, 'pred_segments': b, 'pred_captions': c}
                    for a, b, c in zip(outputs_class, outputs_segment, outputs_captions)]
